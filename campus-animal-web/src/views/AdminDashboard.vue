  ...
  <div class="page-container">
    ...
    <el-table :data="animals" stripe style="width: 100%" v-loading="loading">
      ...
      <el-table-column label="操作" width="200" fixed="right">
        <template #default="{ row }">
          <el-button text type="primary" size="small" @click="$router.push(`/animals/${row.id}`)">查看</el-button>
          <el-button text size="small" @click="$router.push(`/admin/animals/${row.id}/edit`)">编辑</el-button>
          <el-popconfirm title="确定删除该动物档案吗？相关打卡也会被删除。" @confirm="handleDelete(row.id)">
            <template #reference>
...
async function fetchList() {
  loading.value = true
  try {
    const res = await api.get('/animals', { params: { page: page.value, size: size.value } })
